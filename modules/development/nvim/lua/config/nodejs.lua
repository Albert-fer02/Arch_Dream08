    ...
    if vim.fn.executable(nix_node_path) == 1 then
        ...
        if vim.v.shell_error == 0 then
            ...
            if major_version and major_version >= 14 then
                ...
                -- Prefer neovim-node-host if available
                local nix_neovim_node_host = vim.fn.expand("~/.nix-profile/bin/neovim-node-host")
                local nix_npm_path = vim.fn.expand("~/.nix-profile/bin/npm")
                if vim.fn.executable(nix_neovim_node_host) == 1 then
                    vim.g.node_host_prog = nix_neovim_node_host
                elseif vim.fn.executable(nix_npm_path) == 1 then
        ...
        if vim.fn.executable(path) == 1 then
            ...
            if vim.v.shell_error == 0 then
                ...
                if major_version and major_version >= 18 then
                    -- Prefer neovim-node-host if present alongside system node
...
function M.setup(opts)
    ...
    local success, version = setup_nodejs(opts.silent)
